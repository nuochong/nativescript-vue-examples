...
export default {
  ...
  computed: {
    ...
    computedSwipeOpenTriggerProperties() {
      return side => ({
        ...(this.optionsInternal[side].swipeOpenTriggerWidth ? { width: this.optionsInternal[side].swipeOpenTriggerWidth } : {}),
        ...(this.optionsInternal[side].swipeOpenTriggerHeight ? { height: this.optionsInternal[side].swipeOpenTriggerHeight } : {}),
        [this.optionsInternal[side].axis === 'X' ? 'horizontalAlignment' : 'verticalAlignment']: side,
